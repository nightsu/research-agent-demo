...
| HTTP / NDJSON 边界 | `app/api/research/route.ts` | 输入校验、依赖装配、背压、连接取消和唯一终态 |
...
flowchart TB
    ...
    Hook -- JSON request --> Route
    Route -- typed NDJSON --> Hook
...
sequenceDiagram
    ...
    loop until sufficient or bounded
      ...
      A->>M: evaluateSources()
      A-->>H: source.evaluated
      A->>M: assessEvidence()
      A-->>H: conclusion.updated
      opt evidence gap
        A-->>H: gap.detected + follow-up queries
      end
    end
    A-->>H: report.started
    A->>M: generateReport()
    A->>A: validateReportCitations()
    A-->>H: report.completed or research.partial
```

图中从 route 发出的每个箭头实际上都是 Zod 校验后的一行 NDJSON。客户端不能假设网络 chunk 等于一条事件，因此 `useResearchStream` 先用 `TextDecoder` 累积字节，再按换行切分和调用 `decodeEventLine`。
...
`ResearchState` 是服务端编排的最小权威快照：当前 phase、活动 query、累积来源、评估、证据结论、缺口与最终报告。`reduceResearchState()` 拒绝非法迁移，因此模型输出、route 和 UI 都不能自行发明状态。
...
Tavily 的 recoverable search 失败最多重试一次，重试同样计入预算。最终报告也必须经过 `invokeModel()`，所以格式修复不会绕过总步数上限。
...
客户端每次 start 都取消前一请求并使用 generation ID 忽略过期事件；显式 cancel 会终止 fetch。route 把 `request.signal` 传播到 workflow controller，`runResearch` 再为每个模型 / Tavily operation 创建有超时的子 signal。

route 的 `emit()` 在 `ReadableStream.desiredSize <= 0` 时等待 `pull()`，所以慢客户端会暂停下一事件，而不是无限入队。连接取消会拒绝等待者并中止 workflow。

终态只能是 `report.completed`、`research.partial`、`research.cancelled` 或 `research.failed` 之一。route 记录首次终态并拒绝后续 emit；若依赖意外结束却没发终态，`ensureTerminal()` 尝试补一个安全的 `research.failed`。客户端同样拒绝终态后的记录和没有终态就结束的流。
