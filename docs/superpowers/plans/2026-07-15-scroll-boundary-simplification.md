...
- Cancelling a live run at the bottom changed the status to cancelled but did not append a `research.cancelled` printer record: records, `scrollHeight`, and `scrollTop` stayed `5`, `1301`, and `653.5` respectively (bottom distance `0.5`). Therefore the browser run could not verify a cancelled terminal record entering view; the parameterized component test verifies scrolling when such an event is present in `run.events`.
...
- Current regression verification: 18 test files / 289 tests passed. The earlier Task 5 lint and TypeScript checks exited 0, and its production build succeeded after rerunning outside the sandbox because Turbopack's CSS worker needs to bind an internal port.
