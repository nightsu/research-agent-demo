...
At 900 × 800 confirm document/window remains the only persistent vertical owner, the report shell introduces no `overflow-y: auto|scroll`, the compact metadata may remain a row, and there is no horizontal overflow. Then use an exact viewport at or below the 640 px breakpoint, such as 600 × 800, and confirm `.report-meta` stacks as a column with `align-items: flex-start`, a 10 px gap, and no horizontal overflow. Reset the viewport override and stop the server after capture.
