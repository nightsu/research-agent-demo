...
- 采用 AI SDK 6 的一次 `streamText` 和 `partialOutputStream`；具体 output 由模型 capability 选择原生 `Output.object({ schema: reportSchema })` 或 prompted `appendJsonContract + Output.json()`。
...
flowchart TB
    ...
    Native["native\nOutput.object({ schema: reportSchema })\njson_schema"]
    Prompted["prompted\nappendJsonContract + Output.json()\njson_object"]
    Model["一次 AI SDK streamText"]
    ...
    Evidence --> Agent --> Capability
    Capability -->|"true"| Native --> Model
    Capability -->|"false"| Prompted --> Model
...
### 5.2 Provider capability 与 wire contract

Kimi 与 DeepSeek adapter 都必须显式返回冻结的 `ModelCapabilities`，未知模型采用保守默认值。`structuredOutputs: true` 只用于已经验证可接收 `json_schema` 的模型；`false` 使用 prompt 中的精确 JSON Schema 合约和 `Output.json()`，在 wire 上只请求 `json_object`。当前默认 Kimi `kimi-k2.6` 与 DeepSeek 都走保守的 prompted 分支。

两个分支都必须满足同一产品语义：正常报告只有一次 `streamText` 调用、都产生真实 partial、都在结束时返回同一次调用的 final output。prompted partial 进入 UI 前先经过本地 deep-partial Zod schema；暂时无效的快照被跳过，不产生 delta。最终 output 无论来自哪条分支，都必须通过完整 `reportSchema`、来源 ID 与引用约束。测试既要覆盖高层行为，也要在 OpenAI-compatible transport harness 上断言 `json_schema` / `json_object` 请求体，避免把 SDK mock 当作 provider 兼容证据。

### 5.3 partial report 转 Markdown
