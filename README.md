...
5. `lib/agent/research-events.ts`、`lib/server/research-route.ts`、`components/research/use-research-stream.ts`：两请求协议、类型化 NDJSON、输入校验、背压、取消、终态与客户端解码。
...
8. `components/research/research-plan-review.tsx`、`components/research/research-workbench.tsx` 及其展示组件：计划审核、最终交互和引用导航。

## 测试覆盖与第一版限制

测试覆盖 Plan Review 两请求协议、Approved Plan 校验与重试、状态机合法迁移、事件编码 / 解码、研究 happy path 与失败 / 取消 / 预算分支、provider 结构化输出和引用校验、Tavily 响应边界、`createResearchRoute()` 的背压与唯一终态、客户端分块解码、view model 和主要 UI 行为。
