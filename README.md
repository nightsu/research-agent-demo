...
当前仓库没有可用的供应商与 Tavily 密钥，因此没有把真实 API 冒烟结果当作已验证能力：

| Provider | Model（2026-07-12 默认快照） | Structured plan | Structured report | Tavily | End-to-end |
| --- | --- | --- | --- | --- | --- |
| Kimi | `kimi-k2.6` | 未测试 | 未测试 | 未测试 | 未测试 |
| DeepSeek | `deepseek-v4-flash` | 未测试 | 未测试 | 未测试 | 未测试 |

这张表只描述 live smoke 状态；仓库内 mock / unit / integration tests 不能替代真实供应商与 Tavily 的联调。
...
- 真实 Kimi / DeepSeek / Tavily live smoke 仍未验证；上表状态保持“未测试”。
