...
真实凭据仅保存在被 Git 忽略的本地 `.env.local` 中，没有提交到仓库。2026-07-13 已使用 Kimi + Tavily 完成一次本地 quick live smoke：

| Provider | Model（2026-07-12 默认快照） | Structured plan | Structured report | Tavily | End-to-end |
| --- | --- | --- | --- | --- | --- |
| Kimi | `kimi-k2.6` | 已验证（operation 1，无 repair） | 部分报告已验证 | 已验证（2 轮） | 部分验证（`research.partial`） |
| DeepSeek | `deepseek-v4-flash` | 未测试 | 未测试 | 未测试 | 未测试 |

这张表只描述 live smoke 状态；仓库内 mock / unit / integration tests 不能替代真实供应商与 Tavily 的联调。该结果只验证上述日期和本地配置下观察到的路径，不构成永久的供应商能力保证。
...
- Kimi quick 路径已在两轮 Tavily 搜索后到达 `research.partial`；证据充分时的完整报告和 DeepSeek live smoke 仍未验证。
