...
它把一次研究拆成可检查的阶段：规划问题、人工审核并修订计划、搜索网页、抽取正文、评估来源、判断证据缺口、追加搜索、生成带引用的报告。界面展示的是经过类型校验的操作与决策摘要，而不是隐藏推理过程。
...
真实凭据仅保存在被 Git 忽略的本地 `.env.local` 中，没有提交到仓库。2026-07-16 已基于 post-fix 本地实现快照（commit `aa3b2d8`）使用 Kimi + Tavily 重新完成一次 quick live smoke：
...
这张表只描述 live smoke 状态；仓库内 mock / unit / integration tests 不能替代真实供应商与 Tavily 的联调。本次 `kimi-k2.6` 以 `structuredOutputs: false` 的 prompted JSON Object 策略运行：报告仍由一次 `streamText` 产生真实 partial，最终通过完整 schema 与引用校验。该结果只验证上述日期、`aa3b2d8` 实现快照和该次本地配置下观察到的路径，不构成永久的供应商能力保证；DeepSeek 仍未做 live smoke。
...
`quick` 与 `deep` 使用同一条执行工作流和 12 次总 operation 预算，每轮都最多保留 6 条结果，执行阶段单次模型 / Tavily 调用超时均为 120 秒。计划生成属于独立的第一段请求；用户批准后的搜索、评估与报告生成才共享这 12 次预算。`quick` 最多搜索 2 轮、读取 6 个来源；`deep` 最多搜索 5 轮、读取 12 个来源。实际流程也可能因报告预留、证据已足够或预算耗尽而提前结束。执行预算的唯一事实来源是 [`lib/agent/limits.ts`](lib/agent/limits.ts)。
...
1. 第一个 `action: plan` 请求校验用户问题，模型返回结构化 Research Plan，并以 `plan.awaiting_approval` 结束。
2. Plan Review 锁定原始问题、时间范围与研究深度；用户可修改目标、子问题和搜索词，或放弃计划。
3. 第二个 `action: execute` 请求再次用 Zod 校验原始输入与 Approved Plan；没有合法计划就不能执行搜索。
4. 应用代码携带 Tavily 凭据执行搜索与正文抽取。
5. 模型只针对应用提供的来源评估相关性、权威性和时效性。
6. 模型判断证据是否足够；若有缺口，产生最多 3 个跟进查询并进入下一轮。
7. 达到证据目标、搜索轮次或操作预算后，模型生成完整或部分报告。
8. 应用拒绝引用未知来源或已被拒绝来源的结论，再通过 NDJSON 输出唯一执行终态事件。
...
服务端以 `application/x-ndjson` 输出一行一个、经过 Zod 判别联合校验的事件，例如 `plan.completed`、`plan.awaiting_approval`、`progress.updated`、`search.completed`、`source.evaluated`、`gap.detected` 与 `report.completed`。`plan.awaiting_approval` 是计划请求的终态，不代表研究已经完成；批准后客户端开启新的执行流并保留计划记录。`progress.updated` 携带执行阶段真实的 operation / search round 当前值和上限；`deriveResearchViewModel()` 读取最后一条 metrics，在进度面板展示 `Agent operations X/Y` 与 `Search rounds X/Y`。
...
5. `lib/agent/research-events.ts` 与 `lib/agent/research-stream-protocol.ts`：事件 wire shape、任意字节 NDJSON、Request / Research Terminal 和报告草稿序号。
6. `lib/server/research-route.ts` 与 `components/research/use-research-stream.ts`：协议 writer / reader adapter、背压、generation、取消和 React 批处理。
7. `app/api/research/route.ts`：符合 Next.js Route 导出规则的薄包装，只装配生产依赖并导出 `POST` / `maxDuration`。
8. `components/research/research-view-model.ts`：从事件日志派生界面数据。
9. `components/research/research-plan-review.tsx`、`components/research/research-workbench.tsx` 及其展示组件：计划审核、最终交互和引用导航。

## 测试覆盖与第一版限制

测试覆盖 Plan Review 两请求协议、Approved Plan 校验与重试、状态机合法迁移、Research Stream Protocol 的任意字节分块 / UTF-8 / 终态 / delta / 本地取消 / 事务 writer、研究 happy path 与失败 / 取消 / 预算分支、provider 结构化输出和引用校验、Tavily 响应边界、Route 与 Hook adapter、view model 和主要 UI 行为。

已知限制：

- 所有运行数据和待审核计划只在当前 React 内存中，刷新即清空，也不能恢复中断的研究。
- 没有账户、数据库、任务队列、跨设备历史、缓存或共享链接。
- 2026-07-16 post-fix 本地实现快照（commit `aa3b2d8`）的 Kimi quick 路径已完成真实草稿流、`report.completed`、正式报告与引用抽屉；这不是对未来模型版本、其他配置或部署环境的永久能力保证，DeepSeek live smoke 仍未验证。
